...
class AttitudinalEmbedding(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, Y):
        ...
        if Y.isna().sum().sum():
            raise ValueError(f"'Y' has empty values:\n{Y}")
        ...
        # first sort X and Y by entity (so as to have the corresponding
        # mapping in the same rows)
    ...
    def transform(self, X):
        ...
        entitiy_col = None
        # check input and convert to matrix
        if isinstance(X, pd.DataFrame):
            if 'entity' not in X.columns:
                raise ValueError(
                    'Input dataframe has to have an \'entity\' column.')
        ...
        try:
            ...
            raise AttributeError(
                'Transformation parameters have not been computed.')
